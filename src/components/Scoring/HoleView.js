...
class HoleView extends Component {
  ...
  render() {
    const {
      hole, playing, holesCount, scoringSession
    } = this.props
          ...
          <tbody>
            {
              playing.map((item) => {
                ...
                return (
                  <tr key={`hole_view_${item.id}`} onClick={() => (scoringId ? null : this.toggleScoring(item.id))}>
                    ...
                    {scoringId
                      ? null
                      : <ScoreRow {...{
                        scoringType, teamEvent, scoreItem, scoringId: item.id
                      }}
                      />
                    }
